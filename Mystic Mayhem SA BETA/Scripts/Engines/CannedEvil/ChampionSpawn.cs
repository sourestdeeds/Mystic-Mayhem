...
//ST
        ...
        public void Respawn()
        {
            ...
            {
           //ST     while (m_Creatures.Count < (100 - (GetSubLevel() * 20)))
                while (m_Creatures.Count < (40 - (GetSubLevel() * 8)))
                {
                    Mobile m = Spawn();
...
//ST
		if( Utility.RandomDouble() > .40 )  //60%
            ...
            {
		//ST	while( m_Creatures.Count < (200 - (GetSubLevel() * 40)) )
			while( m_Creatures.Count < (40 - (GetSubLevel() * 8)) )		//ST
                {
                    Mobile m = Spawn();
