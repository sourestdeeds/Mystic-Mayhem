...
namespace Server.Engines.Harvest
{
	public class Mining : HarvestSystem
	{
		...
		{
			 double skillBase = from.Skills[SkillName.Mining].Base; //change skill here
	               double skillValueM = from.Skills[SkillName.Mining].Value; // Difine a skill value F for fishing and skill name

                        if (skillValueM >= 100.0)//change skill level
                        {
                               if (Utility.RandomDouble() < 0.01)//change chance 2% atm
                               {
                                     switch (Utility.Random(2))
                                     {
                                          case 0: from.AddToBackpack(new RandomBlackRock()); from.SendLocalizedMessage(1072569); break;
                                          case 1: from.AddToBackpack(new RandomBlackRock()); from.SendLocalizedMessage(1072566); break;
                                     }
                               }
                         }
