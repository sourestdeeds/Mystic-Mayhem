...
namespace Server.Gumps
{
	...
	AddBackground(10, 10, _Width - 20, _Height - 20, 3000);
		...
		protected bool AddStoreListing()
		{
			...
			for( int i = _ListingStart; i < _Store.StoreEntries.Count; i++ )
			{
				...
				{
					//then add a button to withdraw into commodity deed, indexed with i + ItemStore.MaxEntries + 1
					//(where ItemStore.MaxEntries is used in the response code to flag that the user is trying
					//to withdraw using a commodity deed) and offset of 1
					AddButton( xoffset + _WithdrawDeedColumn + entry.ButtonX, _Y + entry.ButtonY, 0x4B9, 0x4B9, ItemStore.MaxEntries + i + 1, GumpButtonType.Reply, 0 );
				}
		...
		protected void AddPageButtons()
		{
			...
			if ( _Page > 0 ) 
			{
				AddButton( 20, _Y, 0x15E3, 0x15E7, ItemStore.MaxEntries * 2 + 4, GumpButtonType.Reply, 0 ); 
			}
			else 
			{
			...
			{
				AddButton( _Width - 40, _Y, 0x15E1, 0x15E5, ItemStore.MaxEntries * 2 + 5, GumpButtonType.Reply, 0 ); 
			}
			else 
			{
		...
		protected void AddControlButtons()
		{
			...
			AddButton( 225, _Y + 3, 0x2C88 + ( _Store.LockWithdrawalAmount ? 10 : 0 ), 0x2C89 + ( _Store.LockWithdrawalAmount ? 10 : 0 ), ItemStore.MaxEntries * 2 + 1, GumpButtonType.Reply, 0 );
			
			AddLabel( _Width / 2 + 70 , _Y, 1153, "Add" );
			AddButton( _Width / 2 + 50, _Y + 5, 0x4B9, 0x4BA, ItemStore.MaxEntries * 2 + 2, GumpButtonType.Reply, 0 );
			...
			AddButton( _Width / 2 + 50, _Y + 5, 0x4B9, 0x4BA, ItemStore.MaxEntries * 2 + 3, GumpButtonType.Reply, 0 );
		}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			//toggle lock button
			if( buttonid == ItemStore.MaxEntries * 2 + 1 )
			{
			...
			//add button
			if( buttonid == ItemStore.MaxEntries * 2 + 2 )
			{
			...
			if( buttonid == ItemStore.MaxEntries * 2 + 3 )
			{
			...
			if( buttonid == ItemStore.MaxEntries * 2 + 4 )
			{
				if( _Page > 0 )
				{
			...
			//next page button
			if( buttonid == ItemStore.MaxEntries * 2 + 5 )
			{
				if( _Page < _MaxPages - 1 )
				{
					_Owner.SendGump( new ItemStoreGump( _Owner, _Store, _Page + 1 ) );
			...
			if( buttonid > ItemStore.MaxEntries )
			{
				deed = true;
				buttonid -= ItemStore.MaxEntries;
			}
