...
namespace Solaris.ItemStore
{
	...
	public class ItemStore 	//TODO: use interface that allows for compare and sort functionality
	{
		//the maximum allowed number of entries in a item store.  this is important so the gump can place buttons properly
		public static int MaxEntries{ get{ return 100; } }
		...
		public void WithdrawItem( Mobile from, int amount, int entryindex, bool makedeed, bool resend )
		{
			...
			if( entry.Amount > 0 )
			{
				...
				Item item = entry.Withdraw( WithdrawAmount, makedeed );
		...
		public void FillFromBackpack( Mobile from, bool resendgump )
		{
			...
			List<Item> packitems = RecurseFindItemsInPack( from.Backpack );
			...
			if( ExpelStoreEntries.Count > 0 )
			{
				foreach( StoreEntry entry in ExpelStoreEntries )
				{
					...
					else
					{
						...
						{
							Item item = entry.Withdraw( entry.Amount, true );
				...
				default:
				{
					...
					if( entrycount > 0 )
					{
						for( int i = 0; i < entrycount; i++ )
						{
							...
							//TODO: think about what this will do to expelled items and classes that have been removed from the scripts.... need to find a way to properly clean house
							...
							{
								//TODO: develop some kind of exception handling here... it's probably not gonna save you at this point if there's a problem here, but you can only hope
							}
						}
					}
		...
		//TODO: is there another more accessible version already made?
		public static List<Item> RecurseFindItemsInPack( Container c )
		{
			...
			{
				if( item is Container && !( item is LockableContainer ) || item is LockableContainer && !((LockableContainer)item).Locked )
				{
