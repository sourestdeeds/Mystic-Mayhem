...
{
	//List entry parent class - special kind of entry that contains a list of speciaal item entry blocks.
	public class ListEntry : StoreEntry
	{
		...
		public virtual int MaxAmount{ get{ return 1000; } }
		...
		//default constructor: set up item type, item list entry type, a name, and default size/offset of artwork
		public ListEntry( Type type, Type itemlistentrytype, string name ) : this( type, itemlistentrytype, name, 40, 0, 0 )
		{
		}
		
		//default constructor: set up item type, item list entry type, a name, size/offset of artwork, and no item list entry
		public ListEntry( Type type, Type itemlistentrytype, string name, int height, int x, int y ) : this( type, itemlistentrytype, name, 25, 0, 0, null )
		{
		}
		
		//master constructor: set up item type, item list entry type, a name, size/offset of artwork, and item list entry
		public ListEntry( Type type, Type itemlistentrytype, string name, int height, int x, int y, List<ItemListEntry> itemlistentries ) : base( type, null, name, 0, height, x, y )
		{
		...
		public ListEntry( ListEntry entry ) : this( entry.Type, entry.ItemListEntryType, entry.Name, entry.Height, entry.X, entry.Y, entry.ItemListEntries )
		{
		}
		...
		{
			//show a south-facing metal box
			return new Item( 0x9A8 );
		}
		...
		public override Item Withdraw( int amount, bool forcequantity )
		{
			return null;
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch( version )
			{
				...
				default:
				{
					...
					{
						ItemListEntry entry = (ItemListEntry)Activator.CreateInstance( ScriptCompiler.FindTypeByName( reader.ReadString() ), new object[]{ reader } );
