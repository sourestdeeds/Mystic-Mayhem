...
{
	//this is the base class that is used to store a listable item entry contained within a ListEntry class
		...
		public override List<ItemListEntryColumn> Columns
		{
			get
			{
				if( _Columns == null )
				{
					...
					_Columns.Add( new ItemListEntryColumn( 150, "Color", _PlantHue.ToString() ) );
					
					PlantHueInfo hueinfo = PlantHueInfo.GetInfo( _PlantHue );
					_Columns.Add( new ItemListEntryColumn( 250, "Bright?", hueinfo.IsBright() ? "yes" : "no" ) );
