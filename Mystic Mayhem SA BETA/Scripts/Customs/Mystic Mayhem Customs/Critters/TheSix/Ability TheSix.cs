...
namespace Server
{
	...
	{
		#region MultiFireball
		public static void MultiFireball( Mobile from, Mobile target, int damage )
		{
			MultiFireball( from, target, damage, false );
		}

		public static void MultiFireball( Mobile from, Mobile target, int damage, bool skip )
		{
			...
			new MultiFireballTimer( from, target, damage ).Start();
		}

		public class MultiFireballTimer : Timer
		{
			...
			public MultiFireballTimer( Mobile from, Mobile target, int damage ) : base( TimeSpan.FromSeconds( 1.0 ), TimeSpan.FromSeconds( 1.0 ) )
			{
				m_From = from;
				m_Target = target;
				m_Damage = damage;
			...
			protected override void OnTick()
			{
				...
				{
					AOS.Damage( m_Target, m_From, m_Damage, 0, 100, 0, 0, 0 );
					Stop();
				}
		...
		public static void SoulDrain( Mobile from )
		{
			...
			foreach( Mobile m in from.GetMobilesInRange( 10 ) )
				if ( m != null )
					...
					{
						if ( from.AccessLevel > m.AccessLevel || CanTarget( from, m ) )
		...
		public class FlameWaveTimer : Timer
		{
			...
			public FlameWaveTimer( Mobile from ) : base( TimeSpan.FromMilliseconds( 300.0 ), TimeSpan.FromMilliseconds( 300.0 ) )
			{
				m_From = from.Location;
				m_Map = from.Map;
				m_Count = 0;
				m_Point = new Point3D();
			}
			...
			protected override void OnTick()
			{
				...
				for ( int i = -m_Count; i < m_Count + 1; i++ )
					...
					{
						m_Point.X = m_From.X + i;
						m_Point.Y = m_From.Y + j;
						dist = GetDist( m_From, m_Point );
			...
			for ( int i = 0; i < 8; i++ )
			{
				...
				for ( int j = 0; j < 16; j++ )
				{
					...
					if ( from.CanSee( point ) )
					{
						...
						if ( itemid != 0 )
							new FireField( from, 30, 2, 4, (itemid == 0x398C), point, from.Map );
						else
						{
							new FireField( from, 30, 2, 4, true, point, from.Map );
							new FireField( from, 30, 0, 0, false, point, from.Map );
						}
					}
				}
		...
		public class JaggedLineTimer : Timer
		{
			...
			public JaggedLineTimer( Mobile from, int range, int speed ) : base( TimeSpan.FromMilliseconds( speed ), TimeSpan.FromMilliseconds( speed ) )
			{
				m_From = from;
				m_D = from.Direction;
			...
			protected override void OnTick()
			{
				...
				if ( m_From == null || m_Count > m_MaxCount )
				{
					Stop();
					return;
				}
...
namespace Server.Commands
{
	public partial class TheSixCommands
	{
		...
		{
			CommandSystem.Register( "MultiFireball", AccessLevel.Seer, new CommandEventHandler( MultiFireball_OnCommand ) );
		...
		[Description( "Use the Six's Multiple Fireball attack" )]
		public static void MultiFireball_OnCommand( CommandEventArgs e )
		{
			e.Mobile.BeginTarget( 10, false, TargetFlags.Harmful, new TargetCallback( MultiFireball_CallBack ) );
		}

		public static void MultiFireball_CallBack( Mobile from, object targeted )
		{
			if ( targeted is Mobile )
				Ability.MultiFireball( from, (Mobile)targeted, ((from.Hits + from.Stam + from.Mana) / 3) );
			else
