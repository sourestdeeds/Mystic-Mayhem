	...
	public class ShrinkFunctions 
	{
		public static bool Shrink( Mobile from, object targ, bool restricted )
		{
			...
			else
			{
				...
				if( t is Clone )
				{ 
					errorString = "You dishornor yourself and must be punished.";
					from.Hits /=4;
					from.Stam = 0;
					from.Mana = 0;
					from.BoltEffect( 0 );
				Effects.SendLocationParticles( EffectItem.Create( t.Location, t.Map, EffectItem.DefaultDuration ), 0x3728, 10, 10, 2023 );
					t.Delete();
				...
				else if( t is BaseHire )
				{ 
					errorString = "You cannot shrink hired NPCs.";
				}
				else if( t is Medic )
				{ 
					errorString = "You cannot shrink your medic.";
				}
				else if( t is BaseEscortable )
				{ 
					errorString = "You cannot shrink escortable NPCs.";
				}
				...
				else if ( (t is PackLlama || t is PackHorse || t is Beetle || t is PackMule) && (t.Backpack != null && t.Backpack.Items.Count > 0) ) //ST
				{
